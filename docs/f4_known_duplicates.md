...
`(standard_no, section, heading_trail_hash, paragraph_id)` 로 해소할 것.
...
1. 위 6 쌍이 composite key 로 모두 unique 해지는지 확인
   (각 쌍의 `heading_trail` 가 반드시 다르다는 점을 DOCX 에서 재확인 필요).
2. sub_item (ilvl≥1) 중복 (ISA-300 `(a)`/`(b)`/`(c)` 5+ 회 등) 도 동일 composite key 로 해소되는지 확인 — `parent_paragraph_id` 가 heading_trail_hash 대체 disambiguator 로 충분할 가능성 있음.
3. Qdrant payload schema 에 `heading_trail_hash` 필드 추가 (payload index 없이 sparse)
4. Chunk 의 external ID 를 `{standard_no}:{section}:{heading_trail_hash}:{paragraph_id}` 로 구성하고 Qdrant `id` 필드에 UUID 로 해싱.
