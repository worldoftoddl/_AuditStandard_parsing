...
def iter_body(docx_path: Path) -> Iterator[RawBlock]:
    ...
    counter = _Counter()
    for child in _iter_block_level(body):
...
def _iter_block_level(parent: etree._Element) -> Iterator[etree._Element]:
    ...
    for child in parent:
        ...
        if tag == _TAG_SDT:
            ...
            if content is not None:
                yield from _iter_block_level(content)
        elif tag in (_TAG_P, _TAG_TBL):
            yield child
